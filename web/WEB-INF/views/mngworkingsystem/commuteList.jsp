						...
						<div class="card-box">
							<div class="card-block">
								...
								<div>
									...
									<c:out
										value="부서명 : ${ sessionScope.memberInfo.department.deptName }" />
								</div>
								...
								<div>
									<table class="table table-striped">
										<thead>
											<tr>
												<c:forEach var="days"
													items="${ requestScope.dailyCommuteList }">
										...
										<tbody>
											<tr style="font-size: 8px">
												<c:forEach var="days"
													items="${ requestScope.dailyCommuteList }">
													<c:set var="isSaterDay"
														value="${ fn:contains(days.dayOfWeek, \"토\") }" />
													<c:set var="isSundayDay"
														value="${ fn:contains(days.dayOfWeek, \"일\") }" />
													<c:choose>
														<c:when test="${ isSaterDay }">
															<td><div style="color: #1C7EBB">
																	<c:out value="${ days.dayOfWeek }" />
																</div></td>
														</c:when>
														<c:when test="${ isSundayDay }">
															<td><div style="color: #E94B3B">
																	<c:out value="${ days.dayOfWeek }" />
																</div></td>
														</c:when>
														<c:otherwise>
											...
											<tr style="font-size: 5px">
												<c:forEach var="days"
													...
													<c:set var="isSundayDay"
														value="${ fn:contains(days.dayOfWeek, \"일\") }" />
													<c:set var="isLate"
														value="${ fn:contains(days.lateYn, \"Y\")}" />
													<c:choose>
											...
											<tr style="font-size: 5px">
												<c:forEach var="days"
													...
													<c:set var="isSundayDay"
														value="${ fn:contains(days.dayOfWeek, \"일\") }" />
													<c:set var="isLeaveEarly"
														value="${ fn:contains(days.leaveEarlyYn, \"Y\")}" />
													<c:choose>
														<c:when test="${ isSaterDay || isSundayDay }">
															<td><div style="color: #E94B3B">휴</div></td>
														</c:when>
														<c:when
															test="${ !empty days.checkOutTime && isLeaveEarly}">
															<th><i class="fa fa-dot-circle-o text-danger"></i><br>
