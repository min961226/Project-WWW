...
public class InsertFamilyEventWelfareServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/* 파일업로드를할경우 한번더 utf-8로 인코딩을해줘야함 */
		response.setContentType("text/html; charset=UTF-8");	
		/* 파일 생성 공간을위한 writer */
		PrintWriter out = response.getWriter();
		/* 파일을 구분할 수 있는 콘텐츠타입 선언 */
        ...
        if (contentType != null &&  contentType.toLowerCase().startsWith("multipart/")) {													
            ...
            for (Part part : parts) {																										
                ...
                if(part.getHeader("Content-Disposition").contains("filename=")) {							
                    ...
                    int dot = fileName.lastIndexOf(".");																					//확장자 인덱스 값 추출
					String ext = fileName.substring(dot);																					//확장자명
		...
		/* 파일첨부를 하지 않았을때는 result값을 더해주면안된다. */
		if(resultFileUpload == -1) {																										
			...
			/* 파일을첨부하였고 그 파일첨부가 성공하였을경우 페이지로 이동한다. */
			if(result1 > 0 && result2 > 0 && result3 > 0 ) {																				
				path = "/WEB-INF/views/common/success.jsp";
				request.setAttribute("successCode", "insertFamilyEvent");
			} else {
