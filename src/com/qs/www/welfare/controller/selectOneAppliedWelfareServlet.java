 ...
 */
@WebServlet("/welfare/applied/selectOne")
public class selectOneAppliedWelfareServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (selectedReport.getDocumentNo() == 9) {
			...
			request.setAttribute("selfDevCodeName", selfDevCodeName); // 경조사 코드 이름
			request.setAttribute("selfDevUseDate", itemList.get(2).getItemContent()); // 경조사명
			request.setAttribute("selfDevBill", itemList.get(3).getItemContent()); // 경조금
			request.setAttribute("reason", itemList.get(4).getItemContent()); // 경조금 신청 내용
		}
		...
		if (selectedReport.getDocumentNo() == 10) {
			...
			request.setAttribute("reason", itemList.get(3).getItemContent()); // 경조금 신청 내용
		}
		...
		String path = "/WEB-INF/views/welfare/appliedDetailWelfareList.jsp";
