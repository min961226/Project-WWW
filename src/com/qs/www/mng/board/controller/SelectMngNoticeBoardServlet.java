...
protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//공지사항 목록
		List<MngNoticeDTO> mngnoticeList = new MngNoticeService().selectAllMngNoticeList(selectCriteria);
		for(MngNoticeDTO mngnotice : mngnoticeList) {
			System.out.println(mngnotice);
		}
		
		String path = "";
		if(mngnoticeList != null) {
			path = "/WEB-INF/views/mngboard/noticeBoard.jsp";
			request.setAttribute("selectCriteria", selectCriteria);
			request.setAttribute("mngnoticeList", mngnoticeList);
		} else {
